...
{
    //On vérifie que les données sont bien passées, attention avec les certificats auto-signés qui ne fonctionnent correctement avec NTLM
    $siret = (isset($_POST['siret']) && isset($_POST['email']))?$_POST['siret']:"23350001600040";

    // Initialisation des variables - 23350001600040 38073485500014
    $path_relative = str_replace(lib, '/conf/', __DIR__);
    require_once($path_relative . "parametres.php");
    ...
    if (is_string($parsed_json)) {
        //Après le Hackathon, l'accès au fournisseur de données n'est plus possible, on tape sur la base locale
        $hackathon=preg_match("/401/i", $parsed_json);
        switch ($hackathon) {
            case 1:
                $attention="Le hackathon, c'est fini. On utilise la base de tests locale :)";
                return array('TypeAlerte' => "warning radius", 'Message' => $attention);
            default :
        ...
        if ($NombreDossier == 0) {
            return array('TypeAlerte' => "warning radius", 'Message' => "Aucun dossier trouvé.");
        }
